...
ethernet ports into which subscriber \acp{AS} plug-in. WHIX sites are
connected by wireless radio links in black, and leased 100Mbps or 1Gbps
circuits in orange. The areas enclosed with dotted lines correspond to
the service areas reachable from each site.
%%%%% FIGURE %%%%%
\begin{figure*}
  \centering
...
points of presence and correspond with the same set of red nodes in Figure~\ref{fig:whixmap}. The dashed lines represent the fully connected
...
We emphasize that layer-2 implementation details are \emph{internal} to
...
Routers in \ac{WHIX} establish a \acs{BGP} peering session with the routers at
Mallaig and Sabhal M\`{o}r Ostaig, which act as route reflectors~\cite{rfc4456}.
The use of route-reflectors enables all participating routers to exchange
reachability information without requiring a full mesh ($n^2$) of internal
peering sessions. The presence of \acs{BGP} signalling throughout the \ac{WHIX}
fabric enables the use of multi-protocol extensions~\cite{rfc4760}. Routers can
use extensions to signal a desire to form part of the exchange \acs{LAN}.
...
New physical interfaces can be added to virtual bridges, as needed, so that they
...
HUBS, as the transit provider described in Section~\ref{subsec:hubs}, is also
present at \ac{WHIX} as a member. In addition to public multilateral peering, it
establishes bilateral sessions with those members wishing to announce either a
default route or full Internet routing tables. HUBS forwards those members'
announcements to their other peers and upstreams. In this way transit, and hence
