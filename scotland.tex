...
featureful \acs{MPLS} implementation, of which more below. All
...
The design requirement of functional equivalence to a \acs{MAC} address
learning Ethernet switch is met using \acs{BGP}
signalled \acs{VPLS}~\cite{rfc4761} to create a full set of \acs{LSP}
...
connectivity established, \acs{LDP}~\cite{rfc5036} is used to enable
the carriage of non-IP traffic with \acs{MPLS} labels according to the
topology of the underlying \acs{OSPF} network.

Each router then establishes a \acs{BGP} peering session with the
...
sesions. With \acs{BGP} signalling established throughout the \ac{WHIX}
...
exchange \acs{LAN}, which has a defined route target value and in this
way a fully meshed \acs{VPLS} is created.

Now each router has a virtual bridge interface that forms part of the
exchange \acs{LAN}, and physical interfaces to which members are
...
network so a \acs{MAC} address filter on the port is employed.
